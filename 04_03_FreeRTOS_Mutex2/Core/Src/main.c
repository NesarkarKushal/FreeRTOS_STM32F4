 ...
 * can be used by other Task or same task.. same scenario is done
 * in this condition i,e MUTEX Condition 2

	MUTEX Condition 2:

	A Mutex is being Giving and taking is done in Task A... in Task B only Taking/ receiving
	so depending on the priority only Higher priority task is running ,Here mutex control is not
	given to task B
...
int main(void)
{
  ...
  xTaskCreate(TaskA,"Task a",128,NULL,2,NULL);
